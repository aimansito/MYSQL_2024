...
/* 1. Queremos obtener un listado en el que se muestren los nombres de departamento y el número de empleados de cada uno. 
...
/* ejemplo de clase:
¿Cuantas veces ha sido director un empleado. Aquellos empleados que no hayan dirigido departamentos deben aparecer */

select numem, nomem, count(*), count(dirigir.numempdirec) as 'num veces director/a', count(distinct dirigir.numempdirec), 
	count(dirigir.numdepto) as 'num veces director/a también', count(distinct dirigir.numdepto) as 'num de deptos distintos dirigidos'
from empleados left join dirigir on empleados.numem = dirigir.numempdirec
group by numem;


/* 2. Queremos averiguar si tenemos algún departamento sin dirección, para ello queremos mostrar el nombre de cada departamento 
y el nombre del director actual, para aquellos departamentos que en la actualidad no tengan director, queremos mostrar el nombre 
...
*/
USE gbdturrural2015;
/* 3. Queremos saber el código de las reservas hechas y anuladas este año, el código de casa reservada, la fecha de inicio de estancia y 
la duración de la misma. También queremos que se muestre el importe de la devolución en aquellas que hayan producido dicha devolución.
*/
...
/* 4. Queremos mostrar un listado de todas las casas de la zona 1 en el que se muestre el nombre de casa y el número de reservas que se han 
...
from reservas right join casas on reservas.codcasa = casas.codcasa join zonas on casas.codzona = zonas.numzona
where zonas.numzona = 1
group by casas.codcasa;
/* 5. Queremos elaborar un listado de casas en el que se muestre el nombre de zona y el nombre de la casa. Ten en cuenta que de algunas zonas 
no tenemos todavía ninguna casa en el sistema, queremos que se muestren estas zonas también. */
...
/* 6. Queremos mostrar un listado de productos organizados por categorías y el número de unidades vendidas. Ten en cuenta que algunos productos 
...
/* 7. Dada una promoción determinada, queremos que se muestre el número de productos que hay en cada categoría incluidos en dicha promoción. 
Ten en cuenta que hay algunas promociones que no incluyen productos de todas las categorías, en cuyo caso debe aparecer la categoría 
